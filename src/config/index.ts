...
export const getOpsAddress = (hre: HardhatRuntimeEnvironment): string => {
  const network = hre.network.name;
  const mainnetOps = "0xB3f5503f93d5Ef84b06993a1975B9D21B962892F";

  if (network == "mainnet" || network == "hardhat") return mainnetOps;
  else throw new Error("Ops address for network not found");
};
